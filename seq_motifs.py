...
def plot_filter_seg_heat(filter_outs, out_pdf, whiten=True, drop_dead=True):
    ...
    sns.clustermap(filter_seqs[:, seqs_i], metric=dist, row_cluster=True, col_cluster=True, linewidths=0,
                   xticklabels=False, vmin=hmin, vmax=hmax)
    plt.savefig(out_pdf)
    # out_png = out_pdf[:-2] + 'ng'
    # plt.savefig(out_png, dpi=300)
    plt.close()
...
def get_motif_fig(filter_weights, filter_outs, out_dir, seqs, sample_i=0):
    ...
    for f in range(num_filters):
        ...
        filter_possum(filter_weights[f, :, :][:, :filter_size], 'filter%d' % f, '%s/filter%d_possum.txt' % (out_dir, f),
                      False)
    ...
    subprocess.call(
        '/home/wangyawei/meme-5.2.0/src/tomtom -dist pearson -thresh 0.05 -eps -oc %s/tomtom %s/filters_meme.txt %s' % (
